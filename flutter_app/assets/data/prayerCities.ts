...
export async function getAccuratePrayerTimes(
  ...
  try {
    ...
    const endpoint = coordinates
      ? `https://api.aladhan.com/v1/timings/${datePath}?latitude=${encodeURIComponent(coordinates.latitude)}&longitude=${encodeURIComponent(coordinates.longitude)}&method=${method === 'BahrainOfficial' ? 8 : methodId}`
      : `https://api.aladhan.com/v1/timingsByCity/${datePath}?city=${encodeURIComponent(city.en)}&country=${encodeURIComponent(country.en)}&method=${methodId}`;
