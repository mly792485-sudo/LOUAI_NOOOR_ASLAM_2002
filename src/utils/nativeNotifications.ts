...
export const scheduleAllNativeNotifications = async (
  ...
  try {
    ...
    const preIqamaOffsetMin = settings.preIqamaMinutes ?? 5;
    ...
    for (const scheduleDay of scheduleDays) {
      ...
      scheduleDay.prayers.forEach((prayer) => {
        ...
        if (prayer.name !== "Sunrise" && settings.prePrayerReminder !== false) {
          ...
          if (preTime.getTime() > Date.now()) {
            ...
            notificationsToSchedule.push({
              ...
              title: preData.title,
              body: preData.body,
              schedule: { at: preTime , allowWhileIdle: true, isExactNotification: true },
        ...
        if (settings.adhanReminder) {
          ...
          if (exactTime.getTime() > Date.now()) {
            ...
            notificationsToSchedule.push({
              ...
              sound: adhanSoundFile, // iOS resolves the bundled WAV filename; Android uses the channel sound
              channelId: "prayer_channel_audio_v6",
              iconColor: "#10b981",
        ...
        // C. Reminder before the configured iqama time (default: 5 minutes)
        if (prayer.name !== "Sunrise" && settings.iqamaReminder !== false) {
          const preIqamaTime = new Date(targetDate);
          preIqamaTime.setHours(pHour, pMin + iqamaOffsetMin - preIqamaOffsetMin, 0, 0);

          if (preIqamaTime.getTime() > Date.now()) {
            notificationsToSchedule.push({
              id: notifIdCounter++,
              title: `اقتربت إقامة صلاة ${prayer.arabicName} 🕌`,
              body: `باقي ${preIqamaOffsetMin} دقائق على الإقامة. تهيأ للصلاة في جماعة.`,
              schedule: { at: preIqamaTime , allowWhileIdle: true, isExactNotification: true },
              sound: "adhan_notification.wav",
              channelId: "prayer_channel_audio_v6",
              iconColor: "#10b981",
              actionTypeId: "PRAYER_ALERT_ACTIONS",
              category: "PRAYER_ALERT_CATEGORY",
              extra: { prayer: prayer.name, type: "pre-iqama" },
            });
          }
        }

        // D. Iqama & Sunnah Follow-up (default: 10 mins after adhan)
        if (prayer.name !== "Sunrise" && settings.iqamaReminder !== false) {
          ...
          if (iqamaTime.getTime() > Date.now()) {
            notificationsToSchedule.push({
              ...
              schedule: { at: iqamaTime , allowWhileIdle: true, isExactNotification: true },
              sound: "adhan_notification.wav",
              channelId: "prayer_channel_audio_v6",
              iconColor: "#10b981",
              actionTypeId: "PRAYER_ALERT_ACTIONS",
