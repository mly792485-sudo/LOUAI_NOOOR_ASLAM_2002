 ...
 * The recording is bundled as a compact MP3 for a smaller app repository.
 */
...
/**
 * Plays the bundled MP3 recording. A user gesture is required by iOS for a
...
export async function playAdhanAudio(
  ...
  const candidateUrls = ['./audio/adhan.mp3'];
    ...
    try {
      ...
      if ('mediaSession' in navigator) {
        navigator.mediaSession.metadata = new MediaMetadata({
          title: 'الأذان',
          artist: 'نور الإسلام — الشيخ علي ملا',
          album: 'مواقيت الصلاة',
        });
        navigator.mediaSession.setActionHandler('play', () => { void audio.play(); });
        navigator.mediaSession.setActionHandler('pause', () => audio.pause());
        navigator.mediaSession.setActionHandler('stop', () => stopAdhanAudio());
      }
...
export function stopAdhanAudio(): void {
  if (activeAdhanAudio) {
    ...
    if ('mediaSession' in navigator) {
      ...
      try { navigator.mediaSession.setActionHandler('stop', null); } catch { /* noop */ }
    }
    onPlayStateChangeCallback?.(false);
  }
}
