...
const ARAB_COUNTRIES: Record<string, string> = {
  BH: 'البحرين', DZ: 'الجزائر', KM: 'جزر القمر', DJ: 'جيبوتي', EG: 'مصر',
  IQ: 'العراق', JO: 'الأردن', KW: 'الكويت', LB: 'لبنان', LY: 'ليبيا',
  MR: 'موريتانيا', MA: 'المغرب', OM: 'عُمان', PS: 'فلسطين', QA: 'قطر',
  SA: 'السعودية', SO: 'الصومال', SD: 'السودان', SY: 'سوريا', TN: 'تونس',
...
const STATIONS: RadioStation[] = [
  ...
  },
  {
    ...
    backupUrl: 'https://aloula.sba.sa/live/sunna',
    officialUrl: 'https://aloula.sba.sa/live/sunna',
    icon: '✨'
  },
  {
    id: 'fatwa',
    ...
    url: 'https://radioplus.sba.sa/live/5',
    backupUrl: 'https://radioplus.sba.sa/live/5',
    officialUrl: 'https://radioplus.sba.sa/live/5',
    icon: '🕌'
  }
];
...
export default function MakkahLiveRadioPlayer({ isEn = false, countryCode = 'BH' }: MakkahLiveRadioPlayerProps) {
  ...
  // Discover current Quran streams by country from Radio Browser. Broken
  ...
  useEffect(() => {
    ...
    fetch(endpoint)
      ...
      .then((items: any[]) => {
        ...
        const discovered: RadioStation[] = items
          .filter((item) => item?.url_resolved?.startsWith('https://') && ARAB_COUNTRIES[item.countrycode])
          .slice(0, 60)
          .map((item) => ({
            id: `radio-browser-${item.stationuuid}`,
            name: `${ARAB_COUNTRIES[item.countrycode]} — ${item.name || 'إذاعة القرآن الكريم'}`,
            subtitle: 'بث قرآن مباشر حسب الدولة (يتطلب الإنترنت)',
            url: item.url_resolved,
            backupUrl: item.url_resolved,
            officialUrl: item.homepage?.startsWith('https://') ? item.homepage : undefined,
            icon: '📻',
      ...
      <div className="relative z-10 pt-5 space-y-4">
        ...
        {isError && (
          <div className="p-2.5 rounded-xl bg-rose-950/60 border border-rose-500/30 text-rose-200 text-xs text-center">
            ...
            {selectedStation.officialUrl && (
              <button type="button" onClick={() => window.open(selectedStation.officialUrl, '_blank', 'noopener,noreferrer')} className="mt-2 inline-flex items-center gap-1 rounded-lg border border-rose-300/30 px-2 py-1 font-bold text-rose-100 hover:bg-white/10">
                <ExternalLink className="w-3 h-3" /> {isEn ? 'Open official broadcast' : 'فتح البث الرسمي'}
              </button>
            )}
          </div>
        )}
