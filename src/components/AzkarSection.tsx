...
const CATEGORY_AUDIO_MAP: Record<string, Record<string, string>> = {
  morning: {
    alafasy: '/audio/adhkar_morning.mp3',
  ...
  evening: {
    alafasy: '/audio/adhkar_evening.mp3',
    ghamdi: 'https://download.quranicaudio.com/adhkar/saad_al_ghamdi_adhkar.mp3',
    ajmy: 'https://download.quranicaudio.com/ruqyah/ahmed_bin_ali_al-ajmy_ruqyah.mp3',
    maher: 'https://everyayah.com/data/MaherAlMuaiqly128kbps/002255.mp3',
    basit: 'https://everyayah.com/data/Abdul_Basit_Murattal_192kbps/002255.mp3',
  ...
  sleep: {
    alafasy: '/audio/adhkar_sleep.mp3',
    ghamdi: 'https://download.quranicaudio.com/adhkar/saad_al_ghamdi_adhkar.mp3',
    ajmy: 'https://download.quranicaudio.com/ruqyah/ahmed_bin_ali_al-ajmy_ruqyah.mp3',
    maher: 'https://everyayah.com/data/MaherAlMuaiqly128kbps/002255.mp3',
    basit: 'https://everyayah.com/data/Abdul_Basit_Murattal_192kbps/002255.mp3',
  ...
  tahajjud: {
    alafasy: '/audio/adhkar_witr.mp3',
    ghamdi: '/audio/adhkar_witr.mp3',
    ajmy: '/audio/adhkar_witr.mp3',
    maher: '/audio/adhkar_witr.mp3',
    basit: '/audio/adhkar_witr.mp3',
    dussary: '/audio/adhkar_witr.mp3',
    minshawi: '/audio/adhkar_witr.mp3',
    qatami: '/audio/adhkar_witr.mp3'
  },
  ruqyah: {
...
// Return ordered candidates. Quranic excerpts use exact ayah audio; other
// adhkar fall back to the complete category recording instead of playing an
// unrelated verse or a silent broken URL.
function getItemAudioCandidates(item: ZekrItem, reciter: AzkarReciter, categoryId: string): string[] {
  ...
  if (item.text.includes('اللَّهُ لَا إِلَٰهَ إِلَّا هُوَ الْحَيُّ الْقَيُّومُ') || item.text.includes('آية الكرسي')) {
    candidates.push(`https://everyayah.com/data/${reciter.quranPrefix}/002255.mp3`);
  }
  // Surah Al-Ikhlas (112)
  if (item.text.includes('قُلْ هُوَ اللَّهُ أَحَدٌ')) {
    candidates.push(`https://everyayah.com/data/${reciter.quranPrefix}/112001.mp3`);
  }
  // Surah Al-Falaq (113)
  if (item.text.includes('قُلْ أَعُوذُ بِرَبِّ الْفَلَقِ')) {
    candidates.push(`https://everyayah.com/data/${reciter.quranPrefix}/113001.mp3`);
  }
  // Surah An-Nas (114)
  if (item.text.includes('قُلْ أَعُوذُ بِرَبِّ النَّاسِ')) {
    candidates.push(`https://everyayah.com/data/${reciter.quranPrefix}/114001.mp3`);
  }
  // Baqarah Endings (285)
  if (item.text.includes('آمَنَ الرَّسُولُ بِمَا أُنْزِلَ إِلَيْهِ')) {
    candidates.push(`https://everyayah.com/data/${reciter.quranPrefix}/002285.mp3`);
  }
  // Al-Fatihah
  if (item.text.includes('الْحَمْدُ لِلَّهِ رَبِّ الْعَالَمِينَ')) {
    candidates.push(`https://everyayah.com/data/${reciter.quranPrefix}/001001.mp3`);
  }

  const categoryUrls = CATEGORY_AUDIO_MAP[categoryId] || CATEGORY_AUDIO_MAP.morning;
  const preferred = categoryUrls[reciter.id];
  const localFallback = categoryUrls.alafasy;
  if (preferred) candidates.push(preferred);
  if (localFallback && localFallback !== preferred) candidates.push(localFallback);
  return candidates;
}
...
export default function AzkarSection({ soundEnabled, isEn = false }: AzkarSectionProps) {
  ...
  const handleToggleItemAudio = (item: ZekrItem) => {
    ...
    const audioUrls = getItemAudioCandidates(item, currentReciter, selectedCatId);
      ...
      for (const audioUrl of audioUrls) {
        try {
          ...
          await new Promise<void>((resolve, reject) => {
            const timeout = window.setTimeout(() => reject(new Error('audio timeout')), 10000);
            audio.oncanplaythrough = () => { window.clearTimeout(timeout); resolve(); };
            audio.onerror = () => { window.clearTimeout(timeout); reject(new Error('audio unavailable')); };
            audio.load();
          });
          audioRef.current = audio;
          audio.onloadedmetadata = () => { setAudioDuration(audio.duration || 0); setIsLoadingAudio(false); };
          audio.ontimeupdate = () => { setCurrentTime(audio.currentTime); if (audio.duration) setAudioProgress((audio.currentTime / audio.duration) * 100); };
          audio.onended = () => { setIsPlaying(false); setActivePlayingId(null); setAudioProgress(0); };
          await audio.play();
          setIsPlaying(true);
          setIsLoadingAudio(false);
          return;
        } catch (_) { /* try the next stable/local candidate */ }
      }
      setIsLoadingAudio(false);
      setIsPlaying(false);
      setActivePlayingId(null);
      setAudioError('تعذر تشغيل صوت هذا الذكر من المصادر المتاحة. تحقق من الاتصال ثم أعد المحاولة.');
    })();
  };
  ...
  const handleToggleCategoryAudio = () => {
    ...
    const audioUrls = [categoryUrls[selectedReciterId], categoryUrls.alafasy].filter(Boolean) as string[];
    setIsLoadingAudio(true);
    ...
    void (async () => {
      for (const audioUrl of audioUrls) {
        try {
          ...
          setIsLoadingAudio(false);
          return;
        } catch (_) { /* try local bundled recording */ }
      }
      setIsLoadingAudio(false);
      setIsPlaying(false);
      setActivePlayingId(null);
      setAudioError('تعذر تشغيل تسجيل هذا القسم. أعد المحاولة مع اتصال إنترنت جيد.');
    })();
  };
  ...
  // Notifications dispatch this event after the user taps an adhkar alert.
  // Open the requested category so the reader can start the full recording.
  useEffect(() => {
    const openRequestedCategory = (event: Event) => {
      const category = (event as CustomEvent<{ category?: string }>).detail?.category;
      if (category && azkarData.some((item) => item.id === category)) {
        handleSelectCategory(category);
      }
    };
    window.addEventListener('noor-open-adhkar', openRequestedCategory);
    return () => window.removeEventListener('noor-open-adhkar', openRequestedCategory);
  }, []);
  ...
  return (
    <div className="w-full space-y-5 text-right font-sans">
      ...
      {audioError && (
        <div role="status" className="rounded-2xl border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-xs font-bold text-amber-200 flex items-center justify-between gap-3">
          <span>{audioError}</span>
          <button type="button" onClick={() => setAudioError(null)} className="text-amber-300 underline">إخفاء</button>
        </div>
      )}
