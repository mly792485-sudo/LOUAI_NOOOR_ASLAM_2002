...
export default function PrayerTimesSection({ settings, onUpdateSettings, onTriggerAdhanPreview, isEn: isEnProp }: PrayerTimesSectionProps) {
  ...
  const [sourceType, setSourceType] = useState<'bahrain-official' | 'api' | 'cached-api' | 'fallback'>('bahrain-official');
  ...
  useEffect(() => {
    ...
    const loadTimes = async () => {
      ...
      setSourceType(res.source as 'bahrain-official' | 'api' | 'cached-api' | 'fallback');
  ...
  }, [selectedCountry, selectedCity, settings.calculationMethod, settings.madhhab, settings.prayerOffsets, settings.latitude, settings.longitude, currentTime.getMinutes()]);

  // Keep the visible countdown moving every second without reloading prayer data.
  useEffect(() => {
    if (!nextPrayer) return;
    const [hour, minute] = nextPrayer.time.split(':').map(Number);
    const target = new Date(currentTime);
    target.setHours(hour, minute, 0, 0);
    if (nextPrayer.name === 'Fajr' && target <= currentTime) target.setDate(target.getDate() + 1);
        ...
        <div className="flex items-center justify-end lg:justify-center">
          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-xl border text-[10px] font-extrabold ${
            sourceType === 'bahrain-official'
              ...
              : sourceType === 'api' || sourceType === 'cached-api'
            ...
            <span>
              {sourceType === 'bahrain-official'
                ...
                : sourceType === 'api' || sourceType === 'cached-api'
                  ? (isEn ? (sourceType === 'cached-api' ? 'Cached AlAdhan times' : 'AlAdhan API') : (sourceType === 'cached-api' ? 'آخر مواقيت محفوظة' : 'مصدر المواقيت API'))
