...
export default function IslamicLibrarySection({ isEn = false }: { isEn?: boolean }) {
  ...
  return (
    <section className="w-full rounded-3xl border border-emerald-500/20 bg-white dark:bg-[#091B1F] p-4 sm:p-6 shadow-xl space-y-5" dir={isEn ? 'ltr' : 'rtl'}>
      ...
      {category === 'home' ? (
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-50 p-6 text-center dark:bg-emerald-950/20">
          <BookOpen className="mx-auto mb-3 h-10 w-10 text-emerald-700 dark:text-emerald-300" />
          <h3 className="text-lg font-black text-slate-800 dark:text-slate-100">{isEn ? 'Welcome to the Islamic Library' : 'مرحبًا بك في المكتبة الإسلامية'}</h3>
          <p className="mx-auto mt-2 max-w-xl text-sm leading-7 text-slate-600 dark:text-slate-300">{isEn ? 'Choose a verified section below. Nothing opens automatically; you stay in the app until you choose a source.' : 'اختر قسمًا موثقًا من الأعلى. لن يفتح أي رابط تلقائيًا؛ ستبقى داخل التطبيق حتى تختار المصدر بنفسك.'}</p>
          <div className="mt-5 grid gap-2 sm:grid-cols-3"><button onClick={() => setCategory('quran')} className="rounded-xl bg-emerald-700 px-4 py-3 text-xs font-black text-white">تلاوات وقرآن</button><button onClick={() => setCategory('sources')} className="rounded-xl bg-sky-700 px-4 py-3 text-xs font-black text-white">مصادر موثوقة</button><button onClick={() => setCategory('quiz')} className="rounded-xl bg-amber-500 px-4 py-3 text-xs font-black text-slate-950">ابدأ المسابقة</button></div>
        </div>
      ) : category === 'quiz' ? (
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {filteredItems.map((item) => <article key={item.id} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 dark:border-slate-800 dark:bg-slate-950/40">
            ...
            <div className="mt-3 flex items-center justify-between gap-2 border-t border-slate-200/70 pt-3 text-[10px] text-slate-500 dark:border-slate-800 dark:text-slate-400"><span className="truncate">{item.source}</span><button type="button" onClick={() => void openSource(item.url)} className="inline-flex shrink-0 items-center gap-1 rounded-lg bg-emerald-700 px-2.5 py-1.5 font-bold text-white"><Play className="h-3 w-3" />فتح المصدر <ExternalLink className="h-3 w-3" /></button></div>
          </article>)}
        </div>
      )}
