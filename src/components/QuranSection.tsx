...
// Some offline/API payloads include the basmala in the first ayah while the
// reader also renders it as the page heading. Strip only that duplicated
// prefix for surahs other than Al-Fatihah and At-Tawbah.
const normaliseSurahBasmala = (surah: SurahDetail): SurahDetail => {
  if (surah.number === 1 || surah.number === 9 || !surah.ayahs?.length) return surah;
  const basmala = /^بِسْمِ\s+اللَّهِ\s+الرَّحْمَٰنِ\s+الرَّحِيمِ\s*/;
  const first = surah.ayahs[0];
  if (!basmala.test(first.text)) return surah;
  return {
    ...surah,
    ayahs: surah.ayahs.map((ayah, index) =>
      index === 0 ? { ...ayah, text: ayah.text.replace(basmala, '').trim() } : ayah,
    ),
  };
};
...
export default function QuranSection({ isEn = false }: QuranSectionProps) {
  ...
  const handleSelectSurah = async (surahMeta: SurahMetadata) => {
    ...
    if (offlineSurahs[surahMeta.number] && offlineSurahs[surahMeta.number].ayahs?.length > 0) {
      setSelectedSurah(normaliseSurahBasmala(offlineSurahs[surahMeta.number]));
      return;
    }
    ...
    if (cachedData) {
      try {
        ...
        setSelectedSurah(normaliseSurahBasmala(parsed));
        return;
      } catch (e) {}
    }
    ...
    try {
      ...
      if (data && data.code === 200 && data.data && data.data.ayahs) {
        ...
        setSelectedSurah(normaliseSurahBasmala(fullSurah));
      }
    } catch (err) {
  ...
  if (isMushafOpen) {
    return <MushafPageReader isEn={isEn} onClose={() => setIsMushafOpen(false)} />;
  }
    ...
    <div className="w-full bg-[#061214] border border-emerald-500/20 rounded-3xl p-4 sm:p-6 space-y-6 text-right font-sans shadow-xl">
      ...
      {!selectedSurah ? (
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pb-3 border-b border-emerald-500/20">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-400 text-slate-950 font-black text-xs shadow-lg active:scale-95"
            >
              <BookOpenCheck className="w-4 h-4" />
              <span>{isEn ? 'Open Mushaf pages' : 'فتح المصحف صفحة بصفحة'}</span>
            </button>
        ...
        <div className="space-y-6">
          ...
          <div className="text-center px-4 py-3 bg-gradient-to-b from-[#0B252A] to-[#040C0E] text-white rounded-2xl space-y-1.5 relative overflow-hidden border border-emerald-500/30 shadow-lg">
            <span className="px-2.5 py-0.5 bg-emerald-500/20 border border-emerald-400/30 rounded-full text-[10px] font-bold text-emerald-300">
              السورة رقم {selectedSurah.number} • {selectedSurah.revelationType === 'Meccan' ? '🕋 مكية' : '🕌 مدنية'}
            </span>
            <h3 className="text-2xl font-black text-amber-300 mt-1 font-serif">سورة {selectedSurah.name}</h3>
            <p className="text-[11px] text-emerald-200/80">{selectedSurah.numberOfAyahs} آيات كريمة</p>
            ...
            <button
              ...
              className="mt-2 inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-slate-950 font-black text-[11px] rounded-full shadow-md transition-all cursor-pointer"
            >
          ...
          {/* Audio Player Card */}
          <div className="p-3 bg-[#091B1F] border border-emerald-500/20 rounded-2xl space-y-2 shadow-lg">
            ...
            <div className="flex flex-col md:flex-row items-center gap-2 justify-between" dir="rtl">
              
              {/* Main Play/Pause & Navigation block */}
              <div className="flex items-center gap-1.5 sm:gap-2 w-full md:w-auto justify-start flex-wrap">
                {/* Prev Surah */}
                <button
          ...
          {/* Compact Mushaf-style text page: verses flow together instead of separate cards. */}
