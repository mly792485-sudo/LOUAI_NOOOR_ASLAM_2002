...
import { playAdhanAudio, stopAdhanAudio, playPrePrayerChime } from './utils/adhanPlayer';
...
export default function App() {
  ...
  // When a user taps an adhkar notification, open the relevant in-app category.
  // The recording is then available from the category audio player.
  useEffect(() => {
    if (!Capacitor.isNativePlatform()) return;
    let listener: { remove: () => Promise<void> } | null = null;
    LocalNotifications.addListener('localNotificationActionPerformed', (event) => {
      const type = String(event.notification.extra?.type || '');
      if (type === 'adhan' || type === 'test') {
        setActiveSection('prayers');
        window.setTimeout(() => { void playAdhanAudio('makkah-ali-mulla', 1); }, 250);
        return;
      }
      const category = type === 'morning_azkar'
        ? 'morning'
        : type === 'evening_azkar'
          ? 'evening'
          : type === 'sleep_azkar'
      ...
      window.setTimeout(() => {
        window.dispatchEvent(new CustomEvent('noor-open-adhkar', { detail: { category } }));
      }, 150);
  ...
  // Keep the native app from exposing browser-style image save/copy menus.
  useEffect(() => {
    ...
    const preventUnintendedSelection = (event: Event) => {
      const target = event.target as HTMLElement | null;
    ...
    document.addEventListener('selectstart', preventUnintendedSelection);
    return () => {
      document.removeEventListener('contextmenu', preventImageContextMenu);
      document.removeEventListener('selectstart', preventUnintendedSelection);
    };
  }, []);
  ...
  const [settings, setSettings] = useState<AppSettings>(() => {
    ...
    const defaults = {
      ...
      developerEmail: 'lwya0721@gmail.com',
      whatsappNumber: '+97366903171',
      snapchatUrl: 'https://snapchat.com/t/Zqgsr5Cz',
      instagramUrl: 'https://www.instagram.com/loj.2011?stkn=MW4xemw5ZGdqaWRtbg==',
    ...
    if (stored) {
      try {
        ...
        if (finalSettings.snapchatUrl === 'https://snapchat.com/t/vezdvWWb' || !finalSettings.snapchatUrl) {
          finalSettings.snapchatUrl = 'https://snapchat.com/t/Zqgsr5Cz';
          modified = true;
        }
        if (!finalSettings.developerEmail) {
          finalSettings.developerEmail = 'lwya0721@gmail.com';
          modified = true;
        }
        if (!finalSettings.whatsappNumber) {
          finalSettings.whatsappNumber = '+97366903171';
          modified = true;
        }
        if (!finalSettings.instagramUrl) {
          finalSettings.instagramUrl = 'https://www.instagram.com/loj.2011?stkn=MW4xemw5ZGdqaWRtbg==';
          modified = true;
        }
        if (modified) {
  ...
  useEffect(() => {
    ...
    const loadAndSchedule = async () => {
      const permissionStatus = await checkPermissionsStatus();
  ...
  }, [settings.country, settings.city, settings.calculationMethod, settings.madhhab, settings.prayerOffsets, settings.latitude, settings.longitude, settings.adhanReminder, settings.azkarReminder, settings.prePrayerReminder, settings.preIqamaMinutes, settings.iqamaReminder, settings.fridayKahfReminder, settings.tahajjudReminder]);
  ...
  useEffect(() => {
    ...
    const checkPrayerTimes = async () => {
      ...
      computedPrayers.forEach((p) => {
        ...
        // At prayer time show a silent reminder; the authentic adhan starts after five minutes.
        if (p.name !== 'Sunrise' && p.time === currentHHMM) {
          const uniqueTriggerId = `${todayStr}-${p.name}-${p.time}-time-reminder`;
          if (!triggeredList.includes(uniqueTriggerId)) {
            const info = PRAYERS_INFO[p.name as keyof typeof PRAYERS_INFO];

            // Browser notification at the exact prayer time is intentionally silent.
            if (settings.adhanReminder) {
              if ('Notification' in window && Notification.permission === 'granted') {
                try {
                  new Notification(`حان وقت صلاة ${p.arabicName}`, {
                    body: `حان الآن وقت صلاة ${p.arabicName} في ${settings.city}. سيبدأ الأذان بعد 5 دقائق.\n${info?.supplication || ''}`,
                    icon: settings.appLogoUrl || defaultLogo,
                    dir: 'rtl'
                  });
                } catch (err) {
            ...
            // Keep the exact-time reminder separate from the adhan trigger.
            window.setTimeout(() => {
              playAdhanAudio(settings.selectedAdhanVoice || 'makkah-ali-mulla', 1.0);
              if ('Notification' in window && Notification.permission === 'granted') {
                try {
                  new Notification(`بدأ الآن أذان صلاة ${p.arabicName}`, {
    ...
    return () => clearInterval(interval);
  }, [settings.city, settings.country, settings.calculationMethod, settings.adhanReminder, settings.visualAdhanAlert, settings.soundEnabled, settings.selectedAdhanVoice, settings.prePrayerReminder, settings.prePrayerMinutes, settings.preIqamaMinutes, settings.iqamaReminder, settings.iqamaMinutes, settings.fridayKahfReminder, settings.tahajjudReminder]);
  ...
  // First-run onboarding: request notifications, then precise location for prayer times and Qibla.
  useEffect(() => {
    let cancelled = false;
    const requestFirstRunPermissions = async () => {
      try {
        await requestAllPermissions();
        if (cancelled || (settings.latitude != null && settings.longitude != null)) return;
        const permission = await Geolocation.requestPermissions();
        if (cancelled || permission.location !== 'granted') return;
        ...
        console.warn('تعذر الحصول على موقع المستخدم:', error);
      }
    };
    void requestFirstRunPermissions();
    return () => { cancelled = true; };
  }, []);
